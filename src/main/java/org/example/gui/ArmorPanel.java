...
public class ArmorPanel extends JPanel {
    ...
    private void showMoldeInfo(Armadura armadura) {
        JDialog dialog = new JDialog((Frame) SwingUtilities.getWindowAncestor(this), 
                                    "Molde de Ferraria: " + armadura.getMoldeFerraria(), true);
        dialog.setLayout(new BorderLayout(10, 10));
        dialog.setSize(500, 400);
        dialog.setLocationRelativeTo(this);
        ...
        // Título
        JLabel titleLabel = new JLabel("🔨 " + armadura.getMoldeFerraria(), SwingConstants.CENTER);
        titleLabel.setFont(new Font("Segoe UI", Font.BOLD, 20));
        ...
        // Como obter o molde
        JLabel obterLabel = new JLabel("Como Obter:");
        obterLabel.setFont(new Font("Segoe UI", Font.BOLD, 16));
        obterLabel.setForeground(Color.WHITE);
        obterLabel.setAlignmentX(Component.LEFT_ALIGNMENT);
        dialogContent.add(obterLabel);

        dialogContent.add(Box.createRigidArea(new Dimension(0, 10)));

        JTextArea receitaArea = new JTextArea(armadura.getReceitaMolde());
        receitaArea.setLineWrap(true);
        receitaArea.setWrapStyleWord(true);
        receitaArea.setEditable(false);
        receitaArea.setBackground(new Color(30, 30, 30));
        receitaArea.setForeground(Color.LIGHT_GRAY);
        receitaArea.setFont(new Font("Segoe UI", Font.PLAIN, 14));
        receitaArea.setBorder(new EmptyBorder(10, 10, 10, 10));
        dialogContent.add(receitaArea);

        dialogContent.add(Box.createRigidArea(new Dimension(0, 20)));

        // Como usar na mesa de ferraria
        JLabel usoLabel = new JLabel("Como Usar na Mesa de Ferraria:");
        usoLabel.setFont(new Font("Segoe UI", Font.BOLD, 16));
        usoLabel.setForeground(Color.WHITE);
        usoLabel.setAlignmentX(Component.LEFT_ALIGNMENT);
        dialogContent.add(usoLabel);

        dialogContent.add(Box.createRigidArea(new Dimension(0, 10)));

        String usoInstrucoes = """
        1. Coloque o molde de ferraria no primeiro slot
        2. Coloque a armadura base no segundo slot
        3. Coloque o material de upgrade no terceiro slot
        4. Retire a armadura melhorada
        
        Nota: O processo preserva todos os encantamentos!
        """;

        JTextArea usoArea = new JTextArea(usoInstrucoes);
        usoArea.setLineWrap(true);
        usoArea.setWrapStyleWord(true);
        usoArea.setEditable(false);
        usoArea.setBackground(new Color(30, 30, 30));
        usoArea.setForeground(MinecraftWikiGUI.MINECRAFT_GREEN);
        usoArea.setFont(new Font("Segoe UI", Font.PLAIN, 14));
        usoArea.setBorder(new EmptyBorder(10, 10, 10, 10));
        dialogContent.add(usoArea);
